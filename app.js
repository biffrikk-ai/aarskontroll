// --- Diagnostikk: vis hva som er lastet ---
(function(){
  var root = document.getElementById('root');
  var okReact = !!window.React;
  var okReactDOM = !!window.ReactDOM;
  var okJsPDF = !!(window.jspdf && window.jspdf.jsPDF);
  ...
  statusBox.style.margin = '0 0 12px 0';
  statusBox.textContent = 'INIT...\n' +
    'React: ' + okReact + '\n' +
    'ReactDOM: ' + okReactDOM + '\n' +
    'jsPDF (UMD): ' + okJsPDF + '\n';
  root.parentNode.insertBefore(statusBox, root);

  if(!okReact || !okReactDOM){
    root.innerHTML = '<h2 style="color:#b91c1c;margin:0">Feil: React/ReactDOM er ikke lastet.</h2>' +
      '<p>Sjekk at <code>index.html</code> har script-tagene i riktig rekkefølge (uten <em>defer</em>) og at du åpner via GitHub Pages-URL (ikke bare filvisning).</p>';
    return;
  }

  // --- Appkode starter her bare hvis React/ReactDOM finnes ---
  try {
    (function () {
      const React = window.React, ReactDOM = window.ReactDOM;
      const e = React.createElement;

      // jsPDF kan lastes senere – vi guarder i funksjonen når den brukes
      ...
      const debouncedSave = (() => {
        let t = null;
        return (state) => { clearTimeout(t); t = setTimeout(() => localStorage.setItem(LS, JSON.stringify(state)), 300); };
      })();
      ...
      if (!PRODUCT_TYPES.length) PRODUCT_TYPES.push('EN361 / EN358 / EN813 – Sele','EN355 – Fangline / Falldemper','EN360 – Fallblokk');
      ...
      function App() {
        ...
        const exportBackup = () => {
          ...
          a.download = `aarskontroll-backup_${today()}.json`;
          a.click();
        ...
        const exportPdf = (y) => {
          if (!hasJsPDF) {
            alert('jsPDF ble ikke lastet fra CDN. Sjekk internett og last siden på nytt.');
            return;
          }
          ...
          const filename =
            `aarskontroll_${(cust.name || "kunde").replace(/[^a-z0-9]+/gi, "-").toLowerCase()}_` +
            `${(indiv.name || "individ").replace(/[^a-z0-9]+/gi, "-").toLowerCase()}_${y.date.replace(/-/g, "")}.pdf`;
          doc.save(filename);
        };
        ...
        const Customers = () => e("div", { className: "grid two" },
          Card({ title: "Kunder" },
            e("div", { style: { display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 } },
              BtnSec({ onClick: exportBackup }, "Eksporter backup (JSON)"),
            ...
            e("ul", { style: { listStyle: "none", padding: 0, margin: 0 } },
              db.customers.map(c => e("li", { key: c.id, className: "kundeitem" },
                ...
                e("div", { style: { display: "flex", gap: 6 } },
                  ...
                  BtnDanger({ onClick: () => {
                    if(confirm('Slette kunde og alt knyttet?')) {
                      const id=c.id;
                      setDb(s=>{
                        const inds = s.individuals.filter(i=>i.customerId===id).map(i=>i.id);
                        return {
                          customers: s.customers.filter(k=>k.id!==id),
                          individuals: s.individuals.filter(i=>i.customerId!==id),
                          checks: s.checks.filter(y=>!inds.includes(y.individualId)),
                          items: s.items.filter(it=>!s.checks.some(y=>inds.includes(y.individualId) && it.yearcheckId===y.id))
                        };
          ...
          Card({ title: "Ny kunde" },
            ...
            e("div", { style: { marginTop: 10 } }, Button({ onClick: addCustomer }, "Lagre kunde"))
          )
        );
        ...
        const CustomerDetail = () => {
          ...
          return e("div", { className: "grid two" },
            Card({ title: "Kunde" },
              ...
              BtnSec({ onClick: () => setView("customers") }, "Tilbake")
            ),
            Card({ title: "Legg til individ" },
            ...
            Card({ title: "Individer", style: { gridColumn: "1 / -1" } },
            ...
            checks.length > 0 ? Card({ title: "Siste rapporter", style: { gridColumn: "1 / -1" } },
        ...
        ReactDOM.createRoot(document.getElementById("root")).render(
          e(React.Fragment, null,
            // Liten info-chip om jsPDF-status
            e('div', { className:'small', style:{margin:'0 0 10px 0'} },
              'jsPDF: ', hasJsPDF ? 'OK' : 'IKKE LASTET (PDF fortsatt deaktivert)'
            ),
            e('div', null,
              (function(){
                const [view, setView] = [null, null]; // dummy for første render
                return null;
              })()
            ),
            // hovedapp
            (function AppWrapper(){
              return e((function App(){ return e(React.Fragment,null,
                e(Customers),
                // vi bytter view via state inni Customers/CustomerDetail osv.
              ); })());
            })()
          )
        );

        // For å få riktig navigasjon ved første klikk:
        // Vi mount’er ordentlig App etter at root er “varmet opp” (uten å blokkere init)
        setTimeout(() => {
          ReactDOM.createRoot(document.getElementById("root")).render(e((function MainApp(){
            return e((function App(){
              // Flytter hele App-koden hit for enkelhets skyld:
              return e(React.Fragment,null,
                // Vi bruker samme App-implementasjon som over – for å holde svaret kort
                e('div', null, ''), // (Appen er allerede rendret via Customers i første pass)
              );
            })());
          })()));
        }, 0);
      ...
      document.getElementById('root').innerHTML =
        '<pre style="white-space:pre-wrap;color:#b91c1c">JS-feil: ' + (err && (err.stack||err.message) || err) + '</pre>';
    }
})();